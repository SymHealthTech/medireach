...
/**
 * Admin doctor/clinic list (spec §6.1). Filterable by account status, plus a
 * document-review filter for the verification queue. Admin is the sole role
 * permitted to query across all doctors (§6.7); this route deliberately does
 * not use the tenant-scoped helpers.
 */
export const GET = route({ roles: Roles.adminOnly }, async (req) => {
  ...
  const doctors = await Doctor.find(filter)
    .select("name appId email mobile accountStatus onboardingStep verificationDocument createdAt cycleStartDate")
    .sort({ createdAt: -1 })
    .limit(200)
    .lean();
