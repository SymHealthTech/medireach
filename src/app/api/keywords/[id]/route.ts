...
import { CustomKeyword, type KeywordMedicine } from "@/models/CustomKeyword";
...
export const PATCH = route<{ id: string }>(
  ...
  async (req, ctx, { id }) => {
    ...
    const { keyword, expansion, medicine } = body as {
      keyword?: string;
      expansion?: string;
    ...
    await CustomKeyword.updateOne(
      ...
      { $set: { keyword, expansion, ...(medicine !== undefined ? { medicine } : {}) } },
    );
